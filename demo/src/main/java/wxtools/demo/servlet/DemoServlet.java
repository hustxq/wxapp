...
import wxtools.demo.handler.ImgHandler;
import wxtools.demo.handler.WhoareyouHandler;
import wxtools.demo.matcher.WhoareyouMatcher;
...
//				String signature = request.getParameter("signature");
			...
			//如果是明文模式，执行以下语句
				...
				// 注意！！每一个规则，必须由end()或者next()结束。不然不会生效。
				// end()是指消息进入该规则后不再进入其他规则。 而next()是指消息进入了一个规则后，如果满足其他规则也能进入，处理。
				router.rule().matcher(new WhoareyouMatcher()).interceptor(new DemoInterceptor()).handler(new WhoareyouHandler()).end();
				//router.rule().matcher(new DemoMatcher()).interceptor(new DemoInterceptor()).handler(new DemoHandler()).end();
				router.rule().handler(new ImgHandler()).end();
				// 把消息传递给路由器进行处理
				WxXmlOutMessage xmlOutMsg = router.route(wx);
				if (xmlOutMsg != null)
